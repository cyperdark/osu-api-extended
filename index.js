...
(async () => {
  try {
    ...
    let new_mods = [],
      str_mods = '';
    let my_mods = 'HREZNFDT';
    // let preset = 'EZHDDT(NC HT)HRSD(PF)NFSOFL';
    // if (my_mods.toLowerCase().indexOf('nc') > -1) preset = 'EZHDNCHRNFSOFL';
    let my_mods_arr = my_mods.match(/.{1,2}/g);
    my_mods_arr.filter(r => all_mods[r.toLowerCase()] !== undefined ? new_mods.push({ i: all_mods[r.toLowerCase()], n: r }) : false)

    let sort = new_mods.sort((a, b) => (a.i > b.i) ? 1 : ((b.i > a.i) ? -1 : 0));
    sort.filter(r => str_mods += r.n);

    console.log(all_mods.length, new_mods, sort, str_mods);
    // class Test {
